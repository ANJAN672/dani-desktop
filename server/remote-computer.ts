...
// inside the VM and Dani Bot never exposes another inbound port.

export const REMOTE_CUA_VERSION = "0.20.0";
export const REMOTE_CUA_EXECUTABLE = "/opt/danibot/cua-driver";
export const REMOTE_CUA_SOCKET = "/opt/danibot/run/cua.sock";
export const REMOTE_CUA_SESSION = "danibot";
export const REMOTE_CDP_HELPER = "/opt/danibot/danibot-cdp.mjs";
...
export function ensureRemoteCuaCommand(): string {
  return [
    ...
    '    display=${display:-:0}',
    `    nohup env HOME="$HOME" DISPLAY="$display" CUA_DRIVER_INSTALL_CHANNEL=python_package CUA_DRIVER_RS_TELEMETRY_ENABLED=0 ${REMOTE_CUA_EXECUTABLE} serve --socket ${REMOTE_CUA_SOCKET} --permission-mode standard > /tmp/danibot-cua-driver.log 2>&1 &`,
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  const installer = [
    "set -eu",
    "trap 'rm -f /tmp/danibot-cua-installing' EXIT",
    "sudo mkdir -p /opt/danibot/run",
    'sudo chown -R "$(id -u):$(id -g)" /opt/danibot',
    'arch="$(uname -m)"',
  ...
  const banner = Buffer.from(`  ▦ ${botName}'s computer — Dani Bot`).toString("base64");
