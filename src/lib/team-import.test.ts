...
describe("team import preview", () => {
  ...
  it("previews portable backups and validates their references before confirmation", () => {
    const backup = {
      format: "dani.backup", version: 1, name: "Saved team", exportedAt: 0,
  ...
  it("rejects unsupported and empty files", () => {
    expect(() => teamImportPreview({ format: "dani.team", version: 3, team: {} })).toThrow("not supported");
    expect(() =>
      teamImportPreview({ format: "dani.team", version: 2, team: { name: "Empty", members: [] } }),
    ).toThrow("no members");
  });
