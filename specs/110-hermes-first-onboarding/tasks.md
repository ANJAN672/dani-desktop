...
- [x] T-UI-002 [R-UI-003] Preserve and verify QR pairing, skip, and resume. Verified visually in the development UI; PACKAGED evidence still missing.
- [ ] T-PRIV-001 [R-PRIV-001] Split local profile persistence from consented remote account/analytics work. Profile saving is local-only with no remote sync on the path, analytics is opt-out and nonblocking, and typed input now survives restart; consent/erasure review still open.
