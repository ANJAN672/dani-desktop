export const JOB_STATES = ["ready", "running", "waiting_approval", "verifying", "succeeded", "failed", "paused", "cancelled"] as const;
export type DaniJobState = (typeof JOB_STATES)[number];
export const EFFECT_STATES = ["planned", "authorized", "dispatched", "confirmed", "uncertain", "denied"] as const;
...
export interface DaniEffectPlan { jobId: string; tool: string; target: string; arguments: unknown; idempotencyKey: string; authorizationId?: string }
export interface DaniEvidenceInput { jobId: string; effectId?: string; kind: "tool_result" | "external_read" | "user_confirmation" | "runtime_event"; source: string; observedAt: string; observation: unknown; digest: string }
