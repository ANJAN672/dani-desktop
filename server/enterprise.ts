...
// The layer's only obligation is `register()`, which turns DANI_LICENSE_KEY (legacy OMB_LICENSE_KEY) into
...
export async function loadEnterpriseLayer(
  ...
  const dir = options.dir ?? (process.env.DANI_ENTERPRISE_DIR ?? process.env.OMB_ENTERPRISE_DIR) ?? DEFAULT_DIR;
  const licenseKey = options.licenseKey ?? (process.env.DANI_LICENSE_KEY ?? process.env.OMB_LICENSE_KEY);
  ...
  if (!entry) {
    return oss(
      licenseKey ? `DANI_LICENSE_KEY is set but no enterprise layer exists at ${dir}` : undefined,
    );
  }
  try {
    ...
    if (!licenseKey) return oss("enterprise layer present but DANI_LICENSE_KEY is not set");
...
export function editionStatus(now: number = Date.now()): EditionStatus {
  ...
  return {
    edition: "oss",
    features: [],
    notice: `enterprise layer disabled: DANI_LICENSE_KEY expired on ${current.expiresAt}; renew it to keep enterprise features`,
  };
}
...
export function describeEdition(status: EditionStatus): string {
  if (status.edition === "enterprise") {
    ...
    return `danibot enterprise edition for ${status.customer}${until}: ${status.features.join(", ") || "no features"}`;
  }
  return `danibot open-source edition${status.notice ? ` (${status.notice})` : ""}`;
}
