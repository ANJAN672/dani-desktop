...
export const DANI_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/danibot/latest.json",
);

type DaniSlot = z.output<typeof slotSchema>;
type DaniUi = z.output<typeof uiSchema>;
type DaniCapsule = z.output<typeof capsuleSchema>;

export interface DaniStatusDigest {
...
export interface DaniStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): DaniUi {
  return {
    two_up: twoUp,
...
function sameUi(left: DaniUi, right: DaniUi): boolean {
  return (
...
function unsignedDocument(capsule: DaniCapsule): JsonObject {
...
function validSlotState(slot: DaniSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): DaniCapsule | null {
...
function unknownDigest(
  reason: NonNullable<DaniStatusDigest["reason"]>,
  freshness: DaniStatusDigest["freshness"] = "unknown",
  receipt?: Pick<DaniStatusDigest, "observedAt" | "receiptSha256">,
): DaniStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
...
export function readDaniStatus(
  options: DaniStatusReadOptions = {},
): DaniStatusDigest {
  const raw = privateCache(options.cachePath ?? DANI_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: DaniCapsule | null;
  try {
...
export function openMausStatusSystemPrompt(options: DaniStatusReadOptions = {}): string {
  const status = readDaniStatus(options);
  const receipt = [
