...
Dani Bot has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
...
- The native Electron window and embedded Dani Bot server on GNOME Xorg and GNOME Wayland.
...
Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Dani Bot
...
24.04 GNOME Wayland session. Dani Bot detects Wayland before XWayland when both `WAYLAND_DISPLAY` and
...
automatically. Dani Bot does not capture screen audio, remember the selected monitor after restart, or
...
starts with `--no-overlay`, so Cua's decorative full-screen X11 cursor surface is never created. Dani Bot also
...
The Xorg runtime uses private sockets, standard permission mode, per-action Dani Bot approvals,
...
Run the CLI directly in a terminal, finish its sign-in flow, then restart Dani Bot. If it lives outside the
