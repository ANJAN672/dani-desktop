...
describe("Box create idempotency", () => {
  ...
  it("retries a lost response with the same key and renames the recovered Box", async () => {
    ...
    expect(renameBodies).toEqual([{ name: expect.stringMatching(/^dani-[0-9a-f]{12}-lostres-[0-9a-f]{6}$/) }]);
  });
  ...
  it("reuses the durable key after a 5xx and module restart", async () => {
    ...
    expect(new Set(createKeys).size).toBe(1);
    expect(renameBodies).toEqual([{ name: expect.stringMatching(/^dani-[0-9a-f]{12}-restart-[0-9a-f]{6}$/) }]);
  });
  ...
  it("bounds retries when a lock disappears between EEXIST and owner read", async () => {
    ...
    try {
      ...
      expect(result.error).toMatch(/locked by another Dani Bot process/i);
  ...
  it("never bypasses a live elected reaper when contenders race", async () => {
    ...
    try {
      ...
      expect(firstResult.error).toMatch(/locked by another Dani Bot process/i);
      expect(secondResult.error).toMatch(/locked by another Dani Bot process/i);
