  ...
  const handle = function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopAccess(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this device — enable it in Dani Bot → Settings → Remote access",
      });
    }
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "Dani Bot is not ready on this computer" });
          };
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from Dani Bot was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "Dani Bot did not respond" }
