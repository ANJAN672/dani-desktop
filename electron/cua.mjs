...
//    EmbeddedCuaDriverHost so TCC grants attribute to Dani Bot and the
//    driver inherits them. One prompt, named Dani Bot, out of the box.
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Dani Bot.
...
async function startEmbedded(binary) {
  ...
  // Dani Bot rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Dani Bot`);
  }
