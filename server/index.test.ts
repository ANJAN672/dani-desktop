...
/**
 * Epic-9 workstream D: isolated test servers fail closed on unknown billing,
 * so fixture providers need the same cost acknowledgement the model picker
 * would record before tests about other behaviors can send messages. The
 * spend gate itself is covered by server/provider-spend-gate.test.ts.
 */
const acknowledgeIsolatedSpend = async (
  // `any` body keeps this assignable to every isolatedApi variant in this file.
  isolatedApi: (method: string, path: string, body?: any) => Promise<{ status: number; body: any }>,
  pairs: ReadonlyArray<readonly [string, string]> = [["claude", "claude-sonnet-5"]],
): Promise<void> => {
  for (const [instanceId, model] of pairs) {
    const ack = await isolatedApi("POST", `/api/instances/${instanceId}/spend-acknowledgement`, {
      model,
      billingClass: "unknown",
    });
    if (ack.status !== 200) {
      throw new Error(
        `isolated fixture spend acknowledgement failed for ${instanceId}/${model}: ${ack.status} ${JSON.stringify(ack.body)}`,
      );
    }
  }
};
...
beforeAll(async () => {
  ...
  // Epic-9 workstream D: the spend gate fails closed on unknown billing, so
  // fixture providers need the same cost acknowledgement a user would give
  // in the model picker before message-flow tests can send. The gate itself
  // is covered by server/provider-spend-gate.test.ts; these acknowledgements
  // keep the integration tests testing message flows, not spend behavior.
  for (const [instanceId, model] of [
    ["claude", "claude-sonnet-5"],
    ["claude", "fixture-claude-model"],
    ["codex", "fixture-codex-model"],
    // NOTE: no ack for ghost/* — the ghost fixture is an unresolvable driver
    // by design ("unknown driver, so the registry resolves nothing"); its
    // tests assert the pre-gate driver errors, not message sends.
  ] as const) {
    const ack = await api("POST", `/api/instances/${instanceId}/spend-acknowledgement`, {
      model,
      billingClass: "unknown",
    });
    if (ack.status !== 200) {
      throw new Error(
        `fixture spend acknowledgement failed for ${instanceId}/${model}: ${ack.status} ${JSON.stringify(ack.body)}`,
      );
    }
  }
}, 30_000);
...
describe("harness HTTP API", () => {
  it("reconciles durable working goal cards with scheduler truth after a restart", async () => {
    ...
    expect(room.messages.find(
      ...
      text: "Goal failed: Dani Bot restarted before this goal finished.",
      goalRun: {
        status: "failed",
        detail: "Dani Bot restarted before this goal finished.",
        turnCount: 2,
  ...
  it("exports every visible bot and imports the team without creating a room", async () => {
    ...
    expect((await api("POST", "/api/teams/export", {})).body.team.name).toBe("My Dani Bot Team");
  ...
  it("leaves a failed credential-card continuation on the card without buzzing twice", async () => {
    ...
    try {
      ...
      expect(directCard).toMatchObject({
        kind: "secret",
        text: "Securely provide the OpenAI API key from Dani Bot on your phone or computer. It is never added to chat.",
      });
  ...
  it("keeps credential-card ownership stable while an encrypted phone save is in flight", async () => {
    ...
    try {
      ...
      await acknowledgeIsolatedSpend(isolatedApi);
  ...
  it("never removes an unmanaged container that squats on a bot's exact Local VM name", async () => {
    ...
    try {
      ...
      expect(removed.body.error).toMatch(/not created by Dani Bot.*remove it manually/i);
