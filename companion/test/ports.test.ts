 ...
 * DANI_COMPANION_DIR that pins the sidecar's own data directory inside it,
  ...
  new Promise((resolve) => {
    const child = spawn(process.execPath, [ENTRY], {
      env: {
        ...
        ...(process.env.DANI_COMPANION_DIR ? { DANI_COMPANION_DIR: process.env.DANI_COMPANION_DIR } : {}),
        ...env,
      },
...
describe("port conflicts with the harness", () => {
  it("refuses the harness's own port, and says so", async () => {
    const { code, err } = await start({ DANI_PORT: "9100", DANI_COMPANION_PORT: "9100" });
    expect(code).toBe(1);
    expect(err).toContain("DANI_COMPANION_PORT");
  ...
  it("refuses the webhook receiver's port, and names it", async () => {
    const { code, err } = await start({ DANI_PORT: "9100", DANI_COMPANION_PORT: "9101" });
    expect(code).toBe(1);
  ...
  it("refuses it for the control port too", async () => {
    const { code, err } = await start({
      DANI_PORT: "9100",
      DANI_COMPANION_PORT: "9200",
      DANI_CONTROL_PORT: "9101",
    });
    expect(code).toBe(1);
    expect(err).toContain("DANI_CONTROL_PORT");
    expect(err).toContain("webhook receiver");
  }, 20_000);
  ...
  it("refuses to put the device port and the control port on one socket", async () => {
    const { code, err } = await start({
      DANI_PORT: "9100",
      DANI_COMPANION_PORT: "9300",
      DANI_CONTROL_PORT: "9300",
    });
    expect(code).toBe(1);
    expect(err).toContain("DANI_COMPANION_PORT");
    expect(err).toContain("DANI_CONTROL_PORT");
  ...
  // An explicit DANI_WEBHOOK_PORT moves the receiver, which frees the port
  // above the harness. Refusing it anyway would be refusing a port nothing
  // is on.
  it("follows DANI_WEBHOOK_PORT rather than assuming the port above", async () => {
    const { code, err } = await start({
      DANI_PORT: "9100",
      DANI_WEBHOOK_PORT: "9500",
      DANI_COMPANION_PORT: "9500",
    });
    expect(code).toBe(1);
    expect(err).toContain("webhook receiver");
    ...
    const moved = await start({
      DANI_PORT: "9100",
      DANI_WEBHOOK_PORT: "9500",
      DANI_COMPANION_PORT: "9101",
      DANI_CONTROL_PORT: "9500",
    });
    expect(moved.code).toBe(1);
    expect(moved.err).toContain("DANI_CONTROL_PORT");
    expect(moved.err).not.toContain("DANI_COMPANION_PORT");
  }, 40_000);
});
