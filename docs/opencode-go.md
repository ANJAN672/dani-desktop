...
OpenCode is an optional Dani Bot engine. Dani Bot runs the maintained
...
2. Connect the providers you want in the OpenCode app, or run
   `opencode auth login`.
3. Restart Dani Bot. It reuses OpenCode's existing connections and model
...
Dani Bot does not copy or rewrite `auth.json`. The OpenCode CLI remains the
