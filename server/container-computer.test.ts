...
describe("containerComputerStatus", () => {
  ...
  it("refuses a per-bot container carrying another target's label", async () => {
    ...
    expect(status.problem).toContain("not created by Dani Bot");
  });
...
describe("containerComputerAction", () => {
  it("never removes an exact-name container without Dani Bot ownership labels", async () => {
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
    ...
    await expect(containerComputerAction("remove", fake.run, "linux")).rejects.toThrow(
      /not created by Dani Bot.*remove it manually/i,
    );
    expect(fake.calls).not.toContain(`docker rm -f ${CONTAINER}`);
  });

  it("removes a verified Dani Bot container even when its version labels are stale", async () => {
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      "docker info --format {{.ServerVersion}}": "29\n",
