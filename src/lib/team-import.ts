...
export function teamImportPreview(manifest: unknown): PendingTeamImport {
  ...
  if (root.format === "dani.backup" || root.format === "openmaus.backup") {
  ...
  if (root.format === "dani.package" || root.format === "openmaus.package") return packagePreview(root, manifest);
  if (root.format !== "dani.team" && root.format !== "openmaus.team") throw new Error("This is not a Dani Bot backup, BotMRR playbook or legacy team.");
...
function markdownPackage(markdown: string): unknown {
  ...
  return { format: "dani.package", version: 1, package: pkg };
}
