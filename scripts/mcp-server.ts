#!/usr/bin/env node
// Model Context Protocol (MCP) Server for Dani Bot
...
export function validateBaseUrl(url: string): string {
  ...
  try {
    ...
    throw new Error(`Invalid Dani Bot URL: '${url}'`);
  }
...
const configuredUrl = process.env.DANIBOT_URL || process.env.OPENMAUSBOT_URL ||
...
function requestTimeoutMs(): number {
  const raw = Number(process.env.DANIBOT_MCP_TIMEOUT_MS ?? process.env.OPENMAUSBOT_MCP_TIMEOUT_MS);
...
async function fetchJson(url: string, options: RequestInit = {}): Promise<any> {
  ...
  if (!response.ok) {
    ...
    if (response.status === 403 && !(process.env.DANIBOT_TOKEN ?? process.env.OPENMAUSBOT_TOKEN)?.trim()) {
      throw new Error(
  ...
  try {
    ...
    throw new Error(`Dani Bot API returned a non-JSON response from ${url}`);
  }
}
...
export async function probeBaseUrls(candidates: string[]): Promise<string> {
  ...
  throw new Error(`Could not find a running Dani Bot server. ${failures.join("; ")}`);
}

export async function resolveBaseUrl(): Promise<string> {
  if (discoveredBaseUrl) return discoveredBaseUrl;
  if ((process.env.DANIBOT_TOKEN ?? process.env.OPENMAUSBOT_TOKEN)?.trim() && !configuredUrl) {
    throw new Error("Set DANIBOT_URL or DANI_PORT when using DANIBOT_TOKEN so credentials are never sent during port discovery");
  }
...
export const TOOLS: McpToolDefinition[] = [
  {
    ...
    description: "Check whether the Dani Bot server is reachable.",
    inputSchema: {
      type: "object",
      properties: {},
...
function idArg(args: Record<string, unknown>, key: string): string {
  ...
  if (!/^[\w-]+$/.test(value)) throw new ToolInputError(`${key} is not a valid Dani Bot ID`);
  return value;
}
  ...
  switch (name) {
    ...
    case "create_bot": {
      ...
      if (!isRecord(created?.bot) || typeof created.bot.id !== "string") {
        throw new Error("Dani Bot did not return the created bot");
      }
    ...
    case "update_bot_profile": {
      ...
      if (!isRecord(result?.bot)) {
        throw new Error("Dani Bot did not return the updated bot");
      }
    ...
    case "create_channel": {
      ...
      if (!isRecord(created?.group) || typeof created.group.id !== "string") {
        throw new Error("Dani Bot did not return the created channel");
      }
    ...
    case "update_channel": {
      ...
      if (!isRecord(result?.group)) {
        throw new Error("Dani Bot did not return the updated channel");
      }
    ...
    case "create_task": {
      ...
      if (!isRecord(result?.task) || typeof result.task.threadId !== "string") {
        throw new Error("Dani Bot did not return the created task");
      }
    ...
    case "rename_task": {
      ...
      if (!isRecord(result?.task)) {
        throw new Error("Dani Bot did not return the renamed task");
      }
      return {
        success: true,
        targetType: args.target_type,
...
if (process.argv[1] && (process.argv[1].endsWith("mcp-server.ts") || process.argv[1].endsWith("mcp-server.js"))) {
  ...
  log("Dani Bot MCP server running on stdio");
}
