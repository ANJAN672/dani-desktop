...
export const SECRET_PATHS = [
  ...
  { section: "tts", field: "key", env: "OMB_TTS_KEY", label: "tts.key" },
  { section: "imageGen", field: "key", env: "OMB_OPENAI_IMAGE_KEY", label: "imageGen.key" },
] as const;
...
const DESKTOP_PARENT_VAR = "OMB_DESKTOP_PARENT";
...
function defaultDataDir(env: NodeJS.ProcessEnv): string {
  return env.OMB_DATA_DIR ?? join(env.HOME ?? "~", ".danibot");
}
...
export function importFdInjectedSecrets(
  ...
  for (const { env: name } of SECRET_PATHS) {
