...
describe("private browser control sync", () => {
  it("mirrors a valid server hold into Electron", () => {
    const take = vi.fn();
    expect(applyBrowserControlHold({ type: "danibot:browser-control", botId: "bot-a", held: true }, take)).toBe(true);
  ...
  it("rejects malformed bot ids and ignores unrelated private messages", () => {
    expect(() => applyBrowserControlHold({ type: "danibot:browser-control", botId: "../other", held: true }, () => {}))
      .toThrow(/invalid browser-control hold/);
    expect(applyBrowserControlHold({ type: "danibot:browser-connection" }, () => {})).toBe(false);
  });
});
...
describe("private browser lifecycle sync", () => {
  it("accepts exact bot/profile deletion messages", () => {
    expect(decodeBrowserLifecycleMessage({
      type: "danibot:browser-bot-deleted",
      requestId,
      botId: "bot_A-1",
  ...
  it("rejects malformed lifecycle ids and ignores unrelated messages", () => {
    expect(() => decodeBrowserLifecycleMessage({ type: "danibot:browser-bot-deleted", botId: "../other" }))
      .toThrow(/bot-deleted/);
    expect(() => decodeBrowserLifecycleMessage({ type: "danibot:browser-profile-deleted", partitionId: "work!" }))
      .toThrow(/profile-deleted/);
    expect(() => decodeBrowserLifecycleMessage({ type: "danibot:browser-profile-deleted", partitionId: "guest" }))
      .toThrow(/profile-deleted/);
    expect(() => decodeBrowserLifecycleMessage({
      type: "danibot:browser-profile-deleted",
    ...
    expect(decodeBrowserLifecycleMessage({ type: "danibot:managed-composio" })).toBeNull();
  });
});
