...
if (process.env.FAKE_PI_DUMP) {
  try {
    ...
    // DANI_MCP_CONFIG; read it here so a test can assert the mount contract
    ...
    if (process.env.DANI_MCP_CONFIG) {
      try {
        mcpConfig = JSON.parse(readFileSync(process.env.DANI_MCP_CONFIG, "utf8"));
      } catch {
