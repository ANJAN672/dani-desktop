...
async function verifySandboxedPreload() {
  ...
  try {
    ...
    const exposed = await probe.webContents.executeJavaScript(`({
      dani: typeof window.dani,
      platform: window.dani?.platform,
      browser: typeof window.dani?.browser,
    })`);
    const expectedBrowser = process.platform === "win32" ? "undefined" : "object";
    if (exposed.dani !== "object" || exposed.platform !== process.platform || exposed.browser !== expectedBrowser) {
