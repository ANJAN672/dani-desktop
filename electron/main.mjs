...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] Dani Bot is already running — focusing that window");
  process.exit(0);
}
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every Dani Bot port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen Dani Bot."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen Dani Bot.";
  return (
...
// Bots hand users files as markdown links to paths inside the Dani Bot
...
app.whenReady().then(async () => {
  if (app.isPackaged) {
    try {
      ...
      dialog.showErrorBox(
        "Dani Bot could not start safely",
        error?.message ?? "Another process is using this Dani Bot data folder.",
      );
      app.quit();
      return;
    }
