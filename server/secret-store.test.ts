...
describe("resolveSecretStore", () => {
  ...
  it("recognizes the rebranded DANI_DESKTOP_PARENT marker the same way", () => {
    // index.ts spawns the server with DANI_DESKTOP_PARENT=1 (OMB_* is the
    // legacy alias); missing the primary name would resolve headless and
    // fail closed inside the packaged app.
    const mode = resolveSecretStore({ DANI_DESKTOP_PARENT: "1" });
    expect(mode.kind).toBe("desktop");
    expect(mode.failClosed).toBe(false);
  });
});
...
describe("importFdInjectedSecrets", () => {
  ...
  it("accepts the rebranded primary and legacy env names for the same secret", () => {
    const io = memoryIo({});
    io.readFdImpl = () => `${FIX.tts}\n`;
    const primary: NodeJS.ProcessEnv = { DANI_TTS_KEY_FD: "3" };
    expect(importFdInjectedSecrets(primary, io)).toEqual(["DANI_TTS_KEY"]);
    expect(primary.DANI_TTS_KEY).toBe(FIX.tts);
    const legacyIo = memoryIo({});
    legacyIo.readFdImpl = () => `${FIX.tts}\n`;
    const legacy: NodeJS.ProcessEnv = { OMB_TTS_KEY_FD: "3" };
    expect(importFdInjectedSecrets(legacy, legacyIo)).toEqual(["OMB_TTS_KEY"]);
    expect(legacy.OMB_TTS_KEY).toBe(FIX.tts);
    // loadConfig reads DANI_TTS_KEY ?? OMB_TTS_KEY, so coverage must too.
    expect(secretCoveredByEnv("tts.key", { DANI_TTS_KEY: FIX.tts })).toBe(true);
    expect(secretCoveredByEnv("tts.key", { OMB_TTS_KEY: FIX.tts })).toBe(true);
    expect(secretCoveredByEnv("tts.key", {})).toBe(false);
  });
