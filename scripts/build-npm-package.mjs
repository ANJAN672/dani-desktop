// Assemble the `danibot` npm package: the self-contained server bundle,
// the built UI, the bundled skills and the CLI, with a package.json of its
// own. `npx danibot serve` then needs Node 24+ and nothing else.
//
...
\`\`\`sh
npx danibot serve                 # starts the server, prints a pairing link + QR
npx danibot serve --tailscale     # HTTPS over your tailnet, no domain needed
npx danibot pair --label "Phone"  # another device later
npx danibot sessions              # who is paired; "sessions revoke <id>" signs one out
\`\`\`
...
console.log(`npm package assembled at ${out} (danibot@${app.version})`);
