...
```sh
pnpm control:dani-agent new-channel --name Review --members BOT_A_ID,BOT_B_ID --url http://127.0.0.1:PORT
# Copy channel.id from the JSON above as CHANNEL_ID.
pnpm control:dani-agent send-channel --channel CHANNEL_ID --text "Reply once" --url http://127.0.0.1:PORT
pnpm control:dani-agent wait --channel CHANNEL_ID --timeout 60 --url http://127.0.0.1:PORT
pnpm control:dani-agent messages --channel CHANNEL_ID --limit 20 --url http://127.0.0.1:PORT
```
