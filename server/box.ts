 ...
 * carrying Dani Bot's exact deterministic name shape leave this boundary;
  ...
  for (const candidate of listed.boxes) {
    ...
    if (legacyOwner && owner && options.adoptLegacy !== false) {
      try {
        ...
        return invalidInventory("Dani Bot could not safely remember this legacy cloud computer's owner — repair it in ascii.dev before continuing");
      }
    }
