...
```sh
pnpm control:dani-agent new-bot --name Probe --url http://127.0.0.1:PORT
# Copy bot.id from the JSON above as BOT_ID.
pnpm control:dani-agent send --bot BOT_ID --text "hello" --url http://127.0.0.1:PORT
pnpm control:dani-agent wait --bot BOT_ID --timeout 30 --url http://127.0.0.1:PORT
pnpm control:dani-agent messages --bot BOT_ID --limit 10 --url http://127.0.0.1:PORT
```
